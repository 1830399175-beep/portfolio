// 用 Edge 无头模式把 作品/小红书穿搭账号/作品集.html 按打印样式导出为横向 A4 PDF。
// 用法：node 导出-小红书穿搭-作品集.js
// 说明：复用人生四格项目验证过的方案 —— 本机 Puppeteer 直接 launch Edge 会失败，
//       改为「手动起 Edge(固定调试端口) + puppeteer.connect」。
// 方案来源：./PDF导出方法说明.md
...
const HTML = 'file://' + path.resolve(ROOT, '作品', '小红书穿搭账号', '作品集.html').replace(/\\/g, '/');
const OUT  = path.join(ROOT, '小红书穿搭账号-作品集.pdf');
const EDGE = 'C:/Program Files (x86)/Microsoft/Edge/Application/msedge.exe';
const PORT = 9230;   // 与小卡超市的端口错开，避免冲突
...
(async () => {
  ...
  // 3) 冻结动画（本页无无限循环动画；入场动画已在打印 CSS 里定格终态）
  await page.addStyleTag({ content: '.reveal{animation:none}' });

  // 4) 校验每个区块的缩放系数与渲染高度（renderedH 应都 <= ~794px = 210mm）
  const fit = await page.evaluate(() => {
    return [...document.querySelectorAll('.hero, .position, .metrics, #gallery, .contact')].map((sec, i) => {
  ...
  fit.forEach(f => console.log(`#${f.idx} ${f.id}  tf=${f.tf}  renderedH=${f.renderedH}px  secScrollH=${f.secScrollH} secClientH=${f.secClientH}`));

  // 5) 导出 PDF（preferCSSPageSize 读取 @page 的 A4 横向尺寸）
  const buf = await page.pdf({ path: OUT, preferCSSPageSize: true, printBackground: true });
  await browser.disconnect();
  edge.kill('SIGKILL');
  console.log('---');
  console.log('WROTE', OUT, (buf.length / 1024).toFixed(0) + ' KB');
  console.log('pages:', countPages(buf));   // 应该 = 5（hero + position + metrics + gallery + contact）
  process.exit(0);
